...
class AutoKeyBroadcaster:
    def __init__(self, root):
        ...
        self.target_window_pattern = tk.StringVar()
        ...
        self.grid_snap_enabled = tk.BooleanVar(value=True)
        self.grid_snap_size = tk.IntVar(value=20)
        self.window_snap_enabled = tk.BooleanVar(value=True)
        self.corner_snap_enabled = tk.BooleanVar(value=True)
        self.running = True
        self.broadcast_all_keys = tk.BooleanVar(value=False)
    ...
    def save_keys(self):
        keys_list = [k.strip() for k in self.keys_entry.get().split(',')]
        keys_file = self.get_profile_keys_file(self.current_profile)
        with open(keys_file, 'w', newline='') as file:
            writer = csv.writer(file)
    ...
    def create_widgets(self):
        ...
        self.profile_label = ttk.Label(profile_frame, text=f"Profile: {self.current_profile}")
        self.profile_label.pack(side=tk.LEFT, padx=5)

        profile_buttons_frame = ttk.Frame(profile_frame)
        profile_buttons_frame.pack(side=tk.RIGHT)

        self.profile_num = tk.StringVar()
        profile_combo = ttk.Combobox(profile_buttons_frame, textvariable=self.profile_num, values=list(range(1, 11)), width=2)
        profile_combo.pack(side=tk.LEFT, padx=2)

        load_profile_btn = ttk.Button(profile_buttons_frame, text="Load", width=5,
                                      command=lambda: self.load_profile(self.profile_num.get()))
        load_profile_btn.pack(side=tk.LEFT, padx=2)

        save_profile_btn = ttk.Button(profile_buttons_frame, text="Save", width=5,
                                      command=lambda: self.save_profile(self.profile_num.get()))
        save_profile_btn.pack(side=tk.LEFT, padx=2)

        default_profile_btn = ttk.Button(profile_buttons_frame, text="Set Default", width=10,
                                         command=lambda: self.set_default_profile(self.profile_num.get()))
        default_profile_btn.pack(side=tk.LEFT, padx=2)

        # Select Frame
        select_frame = ttk.LabelFrame(self.root, text="Target Window")
        select_frame.pack(fill="x", expand=True, padx=10, pady=5)

        ttk.Label(select_frame, text="Select window:").pack(anchor="w", padx=5, pady=5)
        self.window_combo = ttk.Combobox(select_frame, textvariable=self.target_window_pattern, values=self.window_list)
        ...
        self.window_mgmt_frame = ttk.LabelFrame(self.root, text="Window Management")
        ...
        self.viz_canvas = tk.Canvas(self.window_mgmt_frame, bg="black")
        ...
        controls_frame = ttk.Frame(self.window_mgmt_frame)
        controls_frame.pack(fill="x", padx=5, pady=5)

        auto_layout_frame = ttk.Frame(controls_frame)
        auto_layout_frame.pack(side=tk.LEFT, padx=5)

        ttk.Button(auto_layout_frame, text="Auto 1:1", command=lambda: self.auto_layout("1:1")).pack(side=tk.LEFT)
        ttk.Button(auto_layout_frame, text="Auto 16:9", command=lambda: self.auto_layout("16:9")).pack(side=tk.LEFT)
        ttk.Button(auto_layout_frame, text="Auto 21:9", command=lambda: self.auto_layout("21:9")).pack(side=tk.LEFT)
        ttk.Button(auto_layout_frame, text="Auto Fill", command=self.auto_fill).pack(side=tk.LEFT, padx=2)

        borderless_frame = ttk.Frame(controls_frame)
        borderless_frame.pack(side=tk.RIGHT, padx=5)

        self.borderless_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(borderless_frame, text="Borderless",
                        variable=self.borderless_var, command=self.toggle_borderless).pack(side=tk.RIGHT)

        # Broadcast Frame
        broadcast_frame = ttk.LabelFrame(self.root, text="Key Broadcasting")
        broadcast_frame.pack(fill="x", expand=True, padx=10, pady=5)

        broadcast_mode_frame = ttk.Frame(broadcast_frame)
        broadcast_mode_frame.pack(fill="x", padx=5, pady=5)

        self.broadcast_all = ttk.Checkbutton(broadcast_mode_frame, text="Broadcast all keyboard input",
                                             variable=self.broadcast_all_keys, command=self.toggle_broadcast_mode)
        self.broadcast_all.pack(side=tk.LEFT, padx=5)

        self.broadcast_select_keys = tk.BooleanVar(value=True)
        self.broadcast_select = ttk.Checkbutton(broadcast_mode_frame, text="Broadcast select keys",
                                                variable=self.broadcast_select_keys, command=self.toggle_broadcast_mode)
        self.broadcast_select.pack(side=tk.RIGHT, padx=5)

        self.keys_section_frame = ttk.Frame(broadcast_frame)
        self.keys_section_frame.pack(fill="x", padx=5, pady=5)

        ttk.Label(self.keys_section_frame, text="Keys to broadcast (comma separated):").pack(anchor="w", padx=5, pady=5)
        self.keys_entry = ttk.Entry(self.keys_section_frame)
        ...
        keys_btn_frame = ttk.Frame(self.keys_section_frame)
        keys_btn_frame.pack(fill="x", padx=5, pady=5)

        self.load_btn = ttk.Button(keys_btn_frame, text="Load Keys", command=self.load_keys_from_file)
        self.load_btn.pack(side=tk.LEFT, padx=5)

        self.save_btn = ttk.Button(keys_btn_frame, text="Save Keys", command=self.save_keys)
        self.save_btn.pack(side=tk.RIGHT, padx=5)

        self.broadcast_btn = ttk.Button(broadcast_frame, text="Start Broadcasting", command=self.toggle_broadcasting)
        self.broadcast_btn.pack(anchor="center", padx=5, pady=5)

        # Auto Focus Frame
        focus_frame = ttk.LabelFrame(self.root, text="Auto Focus")
        ...
        focus_options_frame = ttk.Frame(focus_frame)
        ...
        # Left button
        self.focus_btn = ttk.Button(focus_options_frame, text="Enable Auto Focus", command=self.toggle_auto_focus)
        self.focus_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

        # Center checkbox
        self.hide_taskbar_btn = ttk.Checkbutton(focus_options_frame, text="Hide taskbar on focus",
                                              variable=self.hide_taskbar_var)
        self.hide_taskbar_btn.grid(row=0, column=1, padx=5, pady=5)

        # Right checkbox
        self.pause_alt_tab_var = tk.BooleanVar(value=True)
        self.pause_alt_tab_btn = ttk.Checkbutton(focus_options_frame, text="Pause on Alt+Tab",
        ...
        log_options_frame = ttk.Frame(log_frame)
        log_options_frame.pack(fill="x", padx=5, pady=2, anchor="w")

        self.logging_checkbox = ttk.Checkbutton(log_options_frame, text="Enable Logging",
        ...
        snap_settings_frame = ttk.Frame(self.window_mgmt_frame)
        snap_settings_frame.pack(fill="x", padx=5, pady=2, before=self.viz_canvas)
        
        ttk.Checkbutton(snap_settings_frame, text="Grid Snap", variable=self.grid_snap_enabled).pack(side=tk.LEFT, padx=2)
        ttk.Label(snap_settings_frame, text="Grid Size:").pack(side=tk.LEFT, padx=20)
        ttk.Spinbox(snap_settings_frame, from_=5, to=50, width=3, textvariable=self.grid_snap_size).pack(side=tk.LEFT, padx=2)
        #ttk.Checkbutton(snap_settings_frame, text="Snap to Windows", variable=self.window_snap_enabled).pack(side=tk.LEFT, padx=5)
        #ttk.Checkbutton(snap_settings_frame, text="Snap to Corners", variable=self.corner_snap_enabled).pack(side=tk.LEFT, padx=5)

        
    def on_keys_entry_focus_out(self, event):
        if self.is_broadcasting and not self.broadcast_all_keys.get():
            # Update the keys list
            self.keys = self.keys_entry.get()
    ...
    def broadcast_selected_keys(self):
        ...
        while self.is_broadcasting and self.running:
            # Get updated keys list on each iteration
            keys = [k.strip() for k in self.keys_entry.get().split(',')]
            
            # Update tracking dictionaries for any new keys
            for key in keys:
            ...
            try:
                ...
                if target_windows and any(window == active_window for window in target_windows):
                    for key in keys:
                            ...
                            if not prev_key_states[key]:  # First press
                                for window in target_windows:
                                    if window != active_window:
                                        win32api.PostMessage(window._hWnd, self.WM_KEYDOWN, ord(key.upper()) if len(key) == 1 else win32con.VK_SPACE, 0)
                        ...
                        elif prev_key_states[key]:  # Key was pressed but now released
                            for window in target_windows:
                                if window != active_window:
                                    win32api.PostMessage(window._hWnd, self.WM_KEYUP, ord(key.upper()) if len(key) == 1 else win32con.VK_SPACE, 0)
            ...
            try:
                ...
                if target_windows and any(window == active_window for window in target_windows):
                    for key in all_keys:
                        ...
                        elif prev_key_states[key] and vk_code:  # Key was pressed but now released
                            for window in target_windows:
                                if window != active_window:
