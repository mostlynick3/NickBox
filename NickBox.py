...
class AutoKeyBroadcaster:
    def __init__(self, root):
        ...
        self.icon_path = None
        self.create_icon_synchronously()
        self.root.geometry("535x960")
        ...
        def check_height():
            if self.root.winfo_height() < 960:
                scrollbar.pack(side="right", fill="y")
            else:
                scrollbar.pack_forget()
            self.root.after(100, check_height)
        ...
        self.mouse_click_pause_time = 0.1
        self.last_mouse_release_time = 0
        ...
        self.current_theme = StringVar(value="sv_ttk_light")
        ...
        self.broadcast_mode = StringVar(value="instant")
        self.round_robin_index = 0
        self.autofocus_paused_for_input = False
        self.broadcast_mode = StringVar(value="instant")

        self.round_robin_loop_enabled = BooleanVar(value=False)
        self.round_robin_loop_delay = IntVar(value=1000)
        self.round_robin_loop_randomize = IntVar(value=100)
    ...
    def start_polling_timer(self):
        self.poll_updates()
        self.root.after(5000, self.start_polling_timer)
    ...
    def toggle_taskbar(self, hide=True):
        ...
        if secondary_taskbar:
            win32gui.ShowWindow(
                secondary_taskbar, win32con.SW_HIDE if hide else win32con.SW_SHOW
            )
    ...
    def on_alt_release(self, event):
        if self.pause_alt_tab_var.get():
            if not keyboard.is_pressed("alt") and self.alt_tab_pressed:
                ...
                self.is_auto_focusing_paused = True
                threading.Timer(
                    self.alt_tab_pause_timer.get(), self.resume_autofocus_after_alt_tab
                ).start()

    def resume_autofocus_after_alt_tab(self):
    ...
    def on_mouse_click(self, x, y, button, pressed):
        if pressed:
            ...
            self.mouse_button_pressed = False
            self.mouse_press_pos = None
            if self.is_auto_focusing and self.is_auto_focusing_paused:
                self.is_auto_focusing_paused = False
    ...
    def get_profile_settings_file(self, profile_name="default"):
        if profile_name == "default":
            ...
            return os.path.join(
                self.profiles_dir, f"profile{profile_name}_settings.json"
            )
    ...
    def save_profile_settings(self):
        ...
        settings = {
            ...
            "broadcast_mode": self.broadcast_mode.get(),
        ...
        if os.path.exists(settings_file):
            try:
                with open(settings_file, "r") as f:
                    ...
                    self.broadcast_all_keys.set(
                        settings.get("broadcast_all_keys", False)
                    )
                    ...
                    self.window_snap_enabled.set(
                        settings.get("window_snap_enabled", True)
                    )
                    self.corner_snap_enabled.set(
                        settings.get("corner_snap_enabled", True)
                    )
                    self.broadcast_mode.set(settings.get("broadcast_mode", "instant"))
                    self.only_on_key_up_var.set(settings.get("only_on_key_up", False))

                    saved_theme = settings.get("theme", "sv_ttk_light")
                    self.current_theme.set(saved_theme)
                    if saved_theme == "sv_ttk_light":
                        sv_ttk.set_theme("light")
                    elif saved_theme == "sv_ttk_dark":
    ...
    def apply_profile_settings(self):
        if self.load_profile_settings():
            if (
                self.target_window_pattern.get()
                and self.target_window_pattern.get() in self.window_list
            ):
    ...
    def load_profile(self, profile_num):
        ...
        if os.path.exists(profile_file) or profile_name == "default":
            ...
            if len(profile_display_name) > 15:
                profile_display_name = profile_display_name[:15] + "..."
            self.profile_label.config(text=f"Profile: {profile_display_name}")
    ...
    def set_current_as_default(self):
        self.save_profile_config(self.current_profile)

        profile_display_name = self.current_profile
        if len(profile_display_name) > 15:
            profile_display_name = profile_display_name[:15] + "..."

        messagebox.showinfo(
            "Success", f"'{self.current_profile}' is now the default profile"
        )

    def setup_toolbar(self):
        toolbar = Frame(self.scrollable_frame)
        toolbar.pack(fill="x", padx=5, pady=5)

        # Profile Menu Button (leftmost)
        profile_btn = Button(toolbar, text="Profile")
        profile_btn.pack(side=LEFT, padx=(15, 15))

        self.separator1 = Label(toolbar, text=f"|")
        self.separator1.pack(side=LEFT, padx=(10, 10))

        self.profile_menu = Menu(self.root, tearoff=0)
        self.profile_menu.add_command(label="Load", command=self.load_profile_dialog)
        self.profile_menu.add_command(label="Save", command=self.save_profile_dialog)
        self.profile_menu.add_command(
            label="Save As", command=self.save_as_profile_dialog
        )
        self.profile_menu.add_separator()
        self.profile_menu.add_command(
            label="Delete", command=self.delete_profile_dialog
        )
        self.profile_menu.add_separator()
        self.profile_menu.add_command(
            label="Set Default", command=self.set_current_as_default
        )

        def show_profile_menu():
            try:
                x = profile_btn.winfo_rootx()
                y = profile_btn.winfo_rooty() + profile_btn.winfo_height()
                self.profile_menu.post(x, y)
            except:
                pass

        profile_btn.config(command=show_profile_menu)

        # Profile Label (next to Profile button)
        profile_display_name = self.current_profile
        if len(profile_display_name) > 15:
            profile_display_name = profile_display_name[:15] + "..."

        self.profile_label = Label(toolbar, text=f"Profile: {profile_display_name}")
        self.profile_label.pack(side=LEFT, padx=(15, 15))

        self.separator1 = Label(toolbar, text=f"|")
        self.separator1.pack(side=LEFT, padx=(10, 10))

        # Settings Menu
        settings_btn = Button(toolbar, text="Settings")
        settings_btn.pack(side=LEFT, padx=(15, 15))
        settings_btn.config(command=self.show_settings_window)

        self.separator1 = Label(toolbar, text=f"|")
        self.separator1.pack(side=LEFT, padx=(10, 10))

        # Preferences Menu
        preferences_btn = Button(toolbar, text="Preferences")
        preferences_btn.pack(side=LEFT, padx=(15, 15))
        preferences_btn.config(command=self.show_preferences_window)
        ...
        try:
            ...
            if profile_file:
                ...
                if profile_name.startswith("profile"):
                    profile_name = profile_name.replace("profile", "").replace(
                        "_settings", ""
                    )

                # Load the profile
                self.load_profile(profile_name if profile_name else "default")
    ...
    def save_as_profile_dialog(self):
        try:
            profile_file = filedialog.asksaveasfilename(
    ...
    def delete_profile_dialog(self):
        try:
            ...
            for file in os.listdir(self.profiles_dir):
                if file.endswith(".json") and "profile" in file:
                    profile_name = file.replace("profile", "").replace(
                        "_settings.json", ""
                    )
                    if profile_name:
            ...
            if not profiles:
                messagebox.showinfo("Info", "No profiles found to delete.")
                return
            ...
            Label(profile_window, text="Select profile to delete:").pack(pady=10)

            profile_var = StringVar()
            profile_combo = Combobox(
                profile_window, textvariable=profile_var, values=profiles
            )
            ...
            def confirm_delete():
                ...
                if not selected_profile:
                    messagebox.showwarning(
                        "Warning", "Please select a profile to delete."
                    )
                    return
                ...
                if result:
                    try:
                        ...
                        settings_file = self.get_profile_settings_file(selected_profile)
                        keys_file = self.get_profile_keys_file(selected_profile)
                        ...
                        messagebox.showinfo(
                            "Success",
                            f"Profile '{selected_profile}' deleted successfully!",
                        )
            ...
            Button(profile_window, text="Delete", command=confirm_delete).pack(pady=10)
            Button(profile_window, text="Cancel", command=profile_window.destroy).pack(
                pady=5
            )

        except Exception as e:
            messagebox.showerror("Error", f"Error accessing profiles: {str(e)}")
    ...
    def show_settings_window(self):
        ...
        # Alt+Tab pause timer setting
        timer_frame = LabelFrame(self.settings_window, text="Alt+Tab Pause Timer")
        timer_frame.pack(fill="x", padx=10, pady=10)

        Label(timer_frame, text="On Alt+Tab, pause autofocus for (seconds):").pack(
            anchor="w", padx=10, pady=5
        )
        timer_spinbox = Spinbox(
            timer_frame, from_=1, to=60, width=10, textvariable=self.alt_tab_pause_timer
        )
        timer_spinbox.pack(anchor="w", padx=10, pady=5)

        # Always on top setting
        top_frame = LabelFrame(self.settings_window, text="Window Behavior")
        top_frame.pack(fill="x", padx=10, pady=10)

        self.temp_always_on_top = BooleanVar(value=self.always_on_top.get())
        always_top_check = Checkbutton(
            top_frame,
            text="Keep NickBox always on top",
            variable=self.temp_always_on_top,
        )
        always_top_check.pack(anchor="w", padx=10, pady=10)
        ...
        def save_settings():
            self.alt_tab_pause_time = self.alt_tab_pause_timer.get()
            self.always_on_top.set(self.temp_always_on_top.get())
            self.toggle_always_on_top()
            messagebox.showinfo("Success", "Settings saved successfully!")
            self.settings_window.destroy()

        Button(button_frame, text="Save", command=save_settings).pack(
            side=RIGHT, padx=5
        )
        Button(button_frame, text="Cancel", command=self.settings_window.destroy).pack(
            side=RIGHT, padx=5
        )

    def show_preferences_window(self):
        if (
            hasattr(self, "preferences_window")
            and self.preferences_window.winfo_exists()
        ):
        ...
        self.preferences_window.geometry("400x300")
        self.preferences_window.resizable(False, False)
        if self.icon_path and os.path.exists(self.icon_path):
            self.preferences_window.iconbitmap(self.icon_path)
        self.preferences_window.attributes("-topmost", True)

        theme_frame = LabelFrame(self.preferences_window, text="Theme")
        theme_frame.pack(fill="x", padx=10, pady=10)

        Label(theme_frame, text="Select theme:").pack(anchor="w", padx=10, pady=5)

        theme_var = StringVar(value=self.current_theme.get())
        theme_options = [("light", "sv_ttk_light"), ("dark", "sv_ttk_dark")]

        for display_name, theme_value in theme_options:
            Radiobutton(
                theme_frame,
        ...
        button_frame = Frame(self.preferences_window)
        button_frame.pack(pady=10)

        def apply_theme():
            selected_theme = theme_var.get()
            try:
                if selected_theme == "sv_ttk_light":
                    sv_ttk.set_theme("light")
                elif selected_theme == "sv_ttk_dark":
                    sv_ttk.set_theme("dark")
        ...
        Button(button_frame, text="Apply", command=apply_theme).pack(side=LEFT, padx=5)
        Button(
            button_frame, text="Cancel", command=self.preferences_window.destroy
    ...
    def create_widgets(self):
        self.setup_toolbar()
        select_frame = LabelFrame(self.scrollable_frame, text="Target Window")
        ...
        self.window_combo = Combobox(
            line_frame, textvariable=self.target_window_pattern, values=self.window_list
        )
        ...
        self.window_mgmt_frame = LabelFrame(
            self.scrollable_frame, text="Window Management"
        )
        ...
        Button(
            auto_layout_frame, text="Auto 1:1", command=lambda: self.auto_layout("1:1")
        ).pack(side=LEFT)
        Button(
            auto_layout_frame,
            text="Auto 16:9",
            command=lambda: self.auto_layout("16:9"),
        ).pack(side=LEFT)
        Button(
            auto_layout_frame,
            text="Auto 21:9",
            command=lambda: self.auto_layout("21:9"),
        ).pack(side=LEFT)
        Button(auto_layout_frame, text="Auto Fill", command=self.auto_fill).pack(
            side=LEFT, padx=2
        )
        ...
        broadcast_mode_frame = LabelFrame(
            self.scrollable_frame, text="Broadcasting Mode"
        )
        broadcast_mode_frame.pack(fill="x", expand=True, padx=10, pady=5)
        mode_radio_frame = Frame(broadcast_mode_frame)
        mode_radio_frame.pack(fill="x", padx=5, pady=5)
        Radiobutton(
            mode_radio_frame,
            text="Instant Broadcast",
            variable=self.broadcast_mode,
            value="instant",
            command=self.on_broadcast_mode_change,
        ).pack(side=LEFT, padx=5)
        Radiobutton(
            mode_radio_frame,
            text="Round Robin",
        ...
        Radiobutton(
            mode_radio_frame,
            text="Round Robin Loop",
            variable=self.broadcast_mode,
            value="round_robin_loop",
            command=self.on_broadcast_mode_change,
        ).pack(side=LEFT, padx=5)
        loop_settings_frame = Frame(broadcast_mode_frame)
        loop_settings_frame.pack(fill="x", padx=5, pady=5)
        self.loop_delay_label = Label(
            loop_settings_frame, text="Loop Delay (ms):", font=("TkDefaultFont", -12)
        )
        self.loop_delay_label.pack(side=LEFT, padx=5)
        self.loop_delay_spinbox = Spinbox(
            loop_settings_frame,
            from_=100,
            to=10000,
            width=6,
            textvariable=self.round_robin_loop_delay,
        )
        self.loop_delay_spinbox.pack(side=LEFT, padx=5)
        self.loop_randomize_label = Label(
            loop_settings_frame, text="± Random (ms):", font=("TkDefaultFont", -12)
        )
        self.loop_randomize_label.pack(side=LEFT, padx=10)
        self.loop_randomize_spinbox = Spinbox(
            loop_settings_frame,
            from_=0,
            to=1000,
            width=6,
            textvariable=self.round_robin_loop_randomize,
        )
        self.loop_randomize_spinbox.pack(side=LEFT, padx=5)
        rr_info_frame = Frame(broadcast_mode_frame)
        rr_info_frame.pack(fill="x", padx=5, pady=2)
        self.rr_status_label = Label(
            rr_info_frame, text="Current target: None", font=("TkDefaultFont", -12)
        )
        self.rr_status_label.pack(side=LEFT, padx=5)
        broadcast_frame = LabelFrame(self.scrollable_frame, text="Key Broadcasting")
        broadcast_frame.pack(fill="x", expand=True, padx=10, pady=(20, 5))
        broadcast_mode_frame_inner = Frame(broadcast_frame)
        broadcast_mode_frame_inner.pack(fill="x", padx=5, pady=5)
        self.broadcast_all = Checkbutton(
            broadcast_mode_frame_inner,
            text="Broadcast all keyboard input",
            variable=self.broadcast_all_keys,
        ...
        self.broadcast_select = Checkbutton(
            broadcast_mode_frame_inner,
            text="Broadcast select keys",
            variable=self.broadcast_select_keys,
            command=self.toggle_broadcast_mode,
        )
        ...
        key_label_frame = Frame(self.keys_section_frame)
        key_label_frame.pack(fill="x", padx=5, pady=5)
        Label(key_label_frame, text="Keys to broadcast (comma separated):").pack(
            side=LEFT, padx=5
        )
        self.keys_entry = Entry(key_label_frame)
        self.keys_entry.pack(side=RIGHT, fill="x", expand=True, padx=5)
        ...
        self.load_btn = Button(
            keys_btn_frame, text="Load Keys", command=self.load_keys_from_file
        )
        self.load_btn.pack(side=LEFT, padx=5)
        self.save_btn = Button(keys_btn_frame, text="Save Keys", command=self.save_keys)
        self.save_btn.pack(side=RIGHT, padx=5)
        self.broadcast_btn = Button(
            broadcast_frame, text="Start Broadcasting", command=self.toggle_broadcasting
        )
        self.broadcast_btn.pack(anchor="center", padx=5, pady=5)
        focus_frame = LabelFrame(self.scrollable_frame, text="Auto Focus")
        focus_frame.pack(fill="x", expand=True, padx=10, pady=15)
        ...
        self.hide_taskbar_btn = Checkbutton(
            focus_options_frame,
            text="Hide taskbar on focus",
        ...
        self.pause_alt_tab_btn = Checkbutton(
            focus_options_frame,
            text="Pause on Alt+Tab",
            variable=self.pause_alt_tab_var,
            command=self.toggle_pause_alt_tab,
        )
        ...
        Checkbutton(
            snap_settings_frame, text="Grid Snap", variable=self.grid_snap_enabled
        ...
        Spinbox(
            ...
            to=50,
            width=3,
            textvariable=self.grid_snap_size,
        ).pack(side=LEFT, padx=2)
        self.use_main_window_check = Checkbutton(
            snap_settings_frame, text="Use Main Window", variable=self.use_main_window
        )
        self.use_main_window_check.pack(side=RIGHT, padx=2)
        self.main_window_combo = Combobox(
            snap_settings_frame,
            textvariable=self.main_window_index,
        ...
        self.update_round_robin_visibility()

    def on_broadcast_mode_change(self):
        self.update_round_robin_visibility()
        self.round_robin_index = 0
        self.update_round_robin_status()
        self.save_profile_settings()

    def update_round_robin_visibility(self):
        is_round_robin = self.broadcast_mode.get() == "round_robin"
        is_round_robin_loop = self.broadcast_mode.get() == "round_robin_loop"

        if is_round_robin_loop:
            self.loop_delay_label.config(state="normal")
            self.loop_delay_spinbox.config(state="normal")
            self.loop_randomize_label.config(state="normal")
            self.loop_randomize_spinbox.config(state="normal")
        else:
            self.loop_delay_label.config(state="disabled")
            self.loop_delay_spinbox.config(state="disabled")
            self.loop_randomize_label.config(state="disabled")
            self.loop_randomize_spinbox.config(state="disabled")

        self.rr_status_label.pack(side=LEFT, padx=5)

    def update_round_robin_status(self):
        target_windows = self.get_target_windows()

        if self.broadcast_mode.get() == "instant":
            pattern = self.target_window_pattern.get()
            window_count = len(target_windows)
            self.rr_status_label.config(
                text=f"Current target: All {pattern} ({window_count} windows) instantly"
            )
        elif self.broadcast_mode.get() == "round_robin":
            if target_windows and self.round_robin_index < len(target_windows):
                window_title = target_windows[self.round_robin_index].title
                if len(window_title) > 30:
                    window_title = window_title[:27] + "..."
                self.rr_status_label.config(
                    text=f"Current target: #{self.round_robin_index + 1} - {window_title}"
                )
            else:
                self.rr_status_label.config(text="Current target: None")
        elif self.broadcast_mode.get() == "round_robin_loop":
            pattern = self.target_window_pattern.get()
            window_count = len(target_windows)
            self.rr_status_label.config(
                text=f"Current target: All {pattern} ({window_count} windows) sequentially"
            )
    ...
    def get_all_windows_windows(self):
        ...
        for window in gw.getAllWindows():
            if (
                ...
                and self.is_window_on_monitor(window, selected_monitor)
            ):
    ...
    def sanitize_keys(self, keys_string):
        ...
        for key in raw_keys:
            if (
                len(key) == 1 and key.lower() in allowed_single_chars
            ) or key.lower() in allowed_special_keys:
    ...
    def update_window_visualization(self):
        ...
        self.screen_scale = min(
            canvas_width / screen_width, canvas_height / screen_height
        )
        ...
        for i, window in enumerate(target_windows):
            if self.is_window_on_monitor(window, selected_monitor):
                ...
                # Round robin indicator: thick yellow border instead of color change
                border_color = (
                    "yellow"
                    if (
                        self.broadcast_mode.get() == "round_robin"
                        and i == self.round_robin_index
                    )
                    else "white"
                )
                border_width = (
                ...
                title = (
                    window.title
                    if len(window.title) < 20
                    else window.title[:17] + "..."
                )
                self.viz_canvas.create_text(
                    (x1 + x2) / 2,
                    (y1 + y2) / 2,
    ...
    def auto_layout(self, aspect_ratio):
        ...
        if use_main_window and 0 <= main_window_idx < window_count:
            ...
            if aspect_ratio == "1:1":
                self.layout_with_main_window_grid(
                    target_windows, screen_width, screen_height
                )
            elif aspect_ratio == "16:9":
                self.layout_with_main_window_widescreen(
                    target_windows, screen_width, screen_height, 16 / 9
                )
            elif aspect_ratio == "21:9":
                self.layout_with_main_window_widescreen(
                    target_windows, screen_width, screen_height, 21 / 9
                )
            else:
                return
        else:
            if aspect_ratio == "1:1":
                ...
                for i, window in enumerate(target_windows):
                    ...
                    self.position_window_correctly(
    ...
    def layout_with_main_window_widescreen(
        self, target_windows, screen_width, screen_height, aspect_ratio
    ):
        if not target_windows:
            return
        window_count = len(target_windows)
        ...
        if secondary_count <= 2:
            ...
            for i in range(bottom_count):
                ...
                y = main_height
                self.position_window_correctly(
                    window, x, y, bottom_width, bottom_height
                )
    ...
    def auto_fill(self):
        ...
        if use_main_window and 0 <= main_window_idx < window_count:
            ...
            if remaining_windows:
                ...
                right_windows_count = max(
                    1, int(len(remaining_windows) * (right_area / total_secondary_area))
                )
                ...
                if right_windows_count > 0:
                    ...
                    for i in range(right_windows_count):
                        ...
                        y = row * cell_height
                        self.position_window_correctly(
                            window, int(x), int(y), int(cell_width), int(cell_height)
                        )
    ...
    def toggle_broadcasting(self):
        if not self.is_broadcasting:
            ...
            broadcast_mode_text = (
                f" ({self.broadcast_mode.get()})"
                if self.broadcast_mode.get() == "round_robin"
                else ""
            )
            self.round_robin_index = 0
            self.update_round_robin_status()
    ...
    def toggle_auto_focus(self):
        if not self.is_auto_focusing:
            ...
            # Unhide taskbar after 2 seconds if it was hidden
            if self.taskbar_hidden:

                def delayed_taskbar_unhide():
                    time.sleep(2)
                    if (
                        not self.is_auto_focusing
                    ):  # Double-check auto focus is still off
                        self.toggle_taskbar(hide=False)

                taskbar_thread = threading.Thread(target=delayed_taskbar_unhide)
                taskbar_thread.daemon = True
    ...
    def broadcast_keys(self):
        while self.is_broadcasting and self.running:
            try:
                ...
                broadcast_all = self.broadcast_all_keys.get()
                if broadcast_all:
    ...
    def start_round_robin_sequence(self, key, target_windows, active_window):
        other_windows = [w for w in target_windows if w != active_window]
        ...
        def delayed_broadcast():
            ...
            vk_code = ord(key.upper()) if len(key) == 1 else self.get_vk_code(key)
            scan_code = win32api.MapVirtualKey(vk_code, 0)
            lParam_down = 1 | (scan_code << 16)
            lParam_up = 1 | (scan_code << 16) | (1 << 30) | (1 << 31)

            for window in other_windows:
                base_delay = self.round_robin_loop_delay.get() / 1000.0
                randomize = self.round_robin_loop_randomize.get() / 1000.0
                actual_delay = base_delay + random.uniform(-randomize, randomize)
                actual_delay = max(0.1, actual_delay)

                time.sleep(actual_delay)

                if not self.is_broadcasting or not self.running:
                    break

                win32api.PostMessage(
                    window._hWnd, self.WM_KEYDOWN, vk_code, lParam_down
                )
                time.sleep(0.001)
                win32api.PostMessage(window._hWnd, self.WM_KEYUP, vk_code, lParam_up)

        thread = threading.Thread(target=delayed_broadcast)
        thread.daemon = True
        thread.start()

    def send_raw_key_event(self, key, target_windows, active_window, is_keydown):
        if is_keydown and self.broadcast_mode.get() in [
            "round_robin",
            "round_robin_loop",
        ]:
            self.autofocus_paused_for_input = True
        ...
        if is_keydown:
            ...
            message_type = self.WM_KEYDOWN
        else:
            lParam = 1 | (scan_code << 16) | (1 << 30) | (1 << 31)
            message_type = self.WM_KEYUP
        ...
        if self.broadcast_mode.get() == "round_robin":
            if is_keydown:
                ...
                self.update_round_robin_status()
            current_target = (
                target_windows[self.round_robin_index] if target_windows else None
            )
            if current_target and current_target != active_window:
                win32api.PostMessage(
                    current_target._hWnd, message_type, vk_code, lParam
                )
        elif self.broadcast_mode.get() == "round_robin_loop":
            if is_keydown:
                self.start_round_robin_sequence(key, target_windows, active_window)
                return
        else:
            for window in target_windows:
                if window != active_window:
                    win32api.PostMessage(window._hWnd, message_type, vk_code, lParam)
    ...
    def broadcast_selected_keys(self):
        prev_key_states = {}
        key_first_press_times = {}
        key_last_repeat_times = {}
        key_repeat_delay = 0.5
        key_repeat_rate = 0.03

        while self.is_broadcasting and self.running:
            keys = [
                k.strip() for k in self.sanitize_keys(self.keys_entry.get()).split(",")
            ]
            ...
            for key in keys:
                if key not in prev_key_states:
                    ...
                    key_first_press_times[key] = 0
                    key_last_repeat_times[key] = 0

            target_windows = self.get_target_windows()
            active_window = gw.getActiveWindow()

            if target_windows and any(
                window == active_window for window in target_windows
            ):
                for key in keys:
                    current_state = keyboard.is_pressed(key)

                    if current_state and not prev_key_states[key]:
                        key_first_press_times[key] = current_time
                        key_last_repeat_times[key] = current_time

                        if self.only_on_key_up_var.get():
                            self.send_raw_key_event(
                                key, target_windows, active_window, is_keydown=True
                            )
                        else:
                            self.send_raw_key_event(
                                key, target_windows, active_window, is_keydown=True
                            ...
                            time.sleep(0.001)
                            self.send_raw_key_event(
                                key, target_windows, active_window, is_keydown=False
                            )

                    elif current_state and prev_key_states[key]:
                        time_held = current_time - key_first_press_times[key]
                        time_since_last_repeat = (
                            current_time - key_last_repeat_times[key]
                        )

                        if (
                            time_held >= key_repeat_delay
                            and time_since_last_repeat >= key_repeat_rate
                        ):
                            key_last_repeat_times[key] = current_time
                    ...
                    elif not current_state and prev_key_states[key]:
    ...
    def auto_focus(self):
        ...
        while self.is_auto_focusing and self.running:
            try:
                if self.is_auto_focusing_paused or self.autofocus_paused_for_input:
                    time.sleep(0.01)
                    continue
                ...
                current_is_target = any(
                    window == current_window for window in target_windows
                )
                    ...
                    if (
                        ...
                        if window != current_window and self.is_window_at_top(
                            ...
                            if current_window and any(
                                ...
                                for key, was_held in currently_held_keys.items():
                                    if was_held:
                                        vk_code = (
                                            ord(key.upper())
                                            if len(key) == 1
                                            else self.get_vk_code(key)
                                        )
                                        if vk_code:
...
if __name__ == "__main__":
    ...
    root = Tk()
    root.option_add("*TCombobox*Listbox*Font", ("TkDefaultFont", 12))
